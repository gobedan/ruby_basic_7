...
class MainMenu
  ...
  def carriage_chooser(train)
    if !train.carriages.empty?
      ...
      loop do
        print ":> > "
        user_input = gets.chomp.to_i
        break train.carriages[user_input - 1] if train.carriages[user_input - 1] && user_input != 0 
        break if user_input == 0 || user_input == 'exit'
      end
    else 
  ...
  def print_single_carriage(carriage)
    if carriage.type == :passenger_carriage
      puts " №#{carriage.id}; Seats avaible: #{carriage.free_seats}; Seats taken: #{carriage.taken_seats}"
    elsif carriage.type == :cargo_carriage
      puts " №#{carriage.id}; Volume avaible: #{carriage.free_volume}; Volume taken: #{carriage.taken_volume}"
    end
  end
